...
from .checks import check_alignment, check_image_validity, check_label_validity
from .checks import check_stac, check_label_type, is_image_georeferenced, is_label_georeferenced
from .geoutils import stack_bands, select_bands, with_connection_retry

logger = logging.getLogger(__name__)

def gdal_translate_copy(src, dst):
    result = subprocess.run(["gdal_translate", "-of", "GTiff", src, dst], capture_output=True, text=True)
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if log_memory:
                ...
                logger.info(f"[{name}] memory change: {delta:.2f} MB (now: {mem_after / 1024**2:.2f} MB)")

            return result
        return wrapper
    return decorator

def load_vector_mask(mask_path: str, skip_layer: str | None = "extent") -> gpd.GeoDataFrame:
    ...
    main_layer = next((layer for layer in layers if skip_layer not in layer.lower()), None)
...
def save_vector_mask(gdf: gpd.GeoDataFrame, 
                     output_path: str, 
...
def validate_pair(image_path, label_path, label_type):
    ...
    with rasterio.open(image_path) as src_image:
        logger.info(f"Validating image in pair")
        ...
        if label_type == "vector":
            logger.info(f"Validating vector label in pair")
            ...
            if not is_image_georeferenced(src_image) or not is_label_georeferenced(label_gdf):
                return {"valid": False,
                        "special_case": False,
            ...
            with rasterio.open(label_path) as src_label:
                ...
                if not label_valid:
                    return {"valid": False,
                            "special_case": False, 
                            "reason": f"Invalid label: {label_msg}"}
                if not is_image_georeferenced(src_image) or not is_label_georeferenced(src_label):
...
def ensure_crs_match(image_path: str, label_path: str, label_type: str, tmp_dir: str) -> Tuple[str, str]:
    """
    ...
    if label_type == 'raster':
        with rasterio.open(label_path) as src_label:
            if image_crs == src_label.crs:
                logger.info(f"label crs matches image crs")
                return image_path, label_path
            else:
                aligned_label_path = Path(tmp_dir) / f"{Path(label_path).stem}_aligned.tif"
                dst_transform, dst_width, dst_height = rasterio.warp.calculate_default_transform(src_label.crs, 
                                                                                                 image_crs, 
                                                                                                 src_label.width, 
                                                                                                 src_label.height, 
                                                                                                 *src_label.bounds)
                ...
                logger.info(f"label crs does not match image crs, reprojecting label")
...
def calculate_overlap(image_path: str,
                      label_path: str,
                      label_type: str) -> Tuple[float, str]:
    """
...
def clip_raster_to_geometry(image_path: str, geometry: box, prefix: str, tmp_dir: str):
   ...
   try:
       if hasattr(geometry, 'geom_type'):
           ...
           gdf = gpd.GeoDataFrame({'id': list(range(len(geometry)))}, geometry=list(geometry), crs=crs)
       
       gdf.to_file(temp_geom_path, driver='ESRI Shapefile')
       del gdf
       
       cmd = ['gdalwarp',
              '-cutline', str(temp_geom_path),
              '-crop_to_cutline',
...
def clip_vector_to_extent(label_path: str, geometry: box, tmp_dir:str) -> gpd.GeoDataFrame:
    """
...
def clip_to_intersection(image_path: str, label_path: str, label_type: str, tmp_dir:str):
    """
    ...
    if intersection is None:
        return None, None
    clipped_image_path = clip_raster_to_geometry(image_path, intersection, 'image', tmp_dir)
    if label_type == 'raster':
        clipped_label_path = clip_raster_to_geometry(label_path, intersection, 'label', tmp_dir)
    elif label_type == 'vector':
...
def create_nodata_mask(image_path: str, nodata_value: Optional[Union[int, float]] = None) -> Optional[gpd.GeoDataFrame]:
    """
    ...
    with rasterio.open(image_path) as src_image:
        ...
        gdf = gdf.dissolve()
    del data, nodata_mask, nodata_mask_flat, mask_array, transform, crs, shapes, geometries
    return gdf

@log_stage(stage_name="apply_nodata_mask", log_memory=True)
def apply_nodata_mask(label_path: str, nodata_mask: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
    """
...
def rasterize_vector(vector: gpd.GeoDataFrame,
                     image_path: str,
                     label_name: str,
                     tmp_dir: str,
    ...
    try:
        vector_clean = vector[~vector.geometry.is_empty & vector.geometry.notnull()].copy()
        ...
        if attr_field and attr_values:
            ...
            if all(isinstance(val, str) for val in vector_clean[attr_field].unique().tolist()):
...
def prepare_vector_labels(label_path: str, 
                          image_path: str, 
                          tmp_dir: str, 
                          attr_field: List[str] = None, 
                          attr_values: list = None):
        ...
        rasterized_label_path = rasterize_vector(label_gdf, image_path, label_name, tmp_dir, attr_field, attr_values)
