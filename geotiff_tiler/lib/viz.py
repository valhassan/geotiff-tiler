...
def _show_image(ax, img: np.ndarray) -> None:
...
def create_dataset_summary_visualization(
    ...
    """
    per_image_cap = max(8, samples_per_split)
    for split in ["trn", "val", "tst"]:
        ...
        try:
            dataset = (
                wds.WebDataset([str(f) for f in selected_tar_files])
                .decode()
                .to_tuple("image_patch.npy", "label_patch.npy", "metadata.json")
            )
            for img, lbl, metadata in islice(
                dataset, images_per_split * per_image_cap * 2
            ):
                patch_metadata = metadata.get("patch_metadata", {})
                image_name = patch_metadata.get("image_name", "unknown")
                bucket = patches_by_image.setdefault(image_name, [])
                image_names.add(image_name)
                if len(bucket) < per_image_cap:
                    bucket.append({"image": img, "label": lbl, "metadata": metadata})
        ...
        for row_idx, patch_data in enumerate(final_patches):
            ...
            _show_image(axes[row_idx, 0], img)
            ...
            axes[row_idx, 0].set_title(
                f"{image_name}\n{sensor} @ ({coords[0]}, {coords[1]})", fontsize=9
            )
            axes[row_idx, 1].set_title(
                f"Label\nClasses: {len(np.unique(lbl_display))}", fontsize=9
            )
        ...
        plt.suptitle(
            f"{prefix} - {split.upper()} Split ({len(selected_images)} images)",
            fontsize=14,
            fontweight="bold",
        )
        plt.tight_layout()
